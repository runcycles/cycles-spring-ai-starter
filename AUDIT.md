...
6. **Commit failure (0.4.0, applies to all three invocation types):** transient (transport / 5xx / 429 incl. bodyless / retryable code) and 401/403 → scheduled on the `CommitRetryEngine` (429 `Retry-After` honored), never thrown in either fail mode; `RESERVATION_EXPIRED` — or a raw 410 status with no parseable error code — → spend recovered via `POST /v1/events` (commit idempotency key, reservation-time subject/action, `recovered_reservation_id` / `recovery_reason` metadata, no `overage_policy`); `RESERVATION_FINALIZED` / `IDEMPOTENCY_MISMATCH` → warn only; other genuine 4xx → the lifecycle first releases the reservation itself (`POST .../release`, reason `commit_rejected_<code>`), then the historical fail-open/fail-closed policy (the only place it still applies to commits) — no commit site releases a second time; any other non-2xx status (e.g. 3xx) → warn only, no throw, no schedule.
...
5. On `RuntimeException` from the wrapped tool → `POST .../release` with reason = `tool-call-failed: <ExceptionClass>`, then re-throw. The commit runs OUTSIDE this catch: a fail-closed genuine-4xx commit rejection propagates as `IllegalStateException` after the lifecycle's own `commit_rejected_<code>` release — never mislabeled as a tool failure, never double-released.
...
Self-review hardening (same release, PR #70): genuine-4xx commit rejections now release in the lifecycle itself (reason `commit_rejected_<code>`) before the fail policy, harmonizing all three commit sites — the tool path's commit moved out of its invocation try/catch (no more misleading `tool-call-failed` double-release). Raw 410 with unparseable body → event fallback (fleet D1); unclassifiable non-4xx statuses (e.g. 3xx) → warn only. Old constructors marked `@Deprecated` (each builds a live journaled engine — see CHANGELOG "Behavior note"); deprecated `CyclesToolGate` constructors now build one engine per gate, not per `wrap()`.
